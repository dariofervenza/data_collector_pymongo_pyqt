...
class AuthWindow(QWidget):
    ...
    def receive_thread_response(self, thread_response):
        ...
        and self.thread_response == "Autenticado":
            ...
            self.mainwindow.main_widget.leer_datos_db()
            self.mainwindow.graficos_widget.token = self.token
            self.mainwindow.graficos_widget.execute_initial_code()
            self.mainwindow.alarmas_widget.token = self.token
            self.mainwindow.alarmas_widget.execute_initial_code()
            self.mainwindow.analytics_widget.token = self.token
            # loop = asyncio.get_event_loop()
            # loop.run_until_complete(self.mainwindow.analytics_widget.read_data_from_db())
            # self.mainwindow.analytics_widget.create_train_test_split()
            self.mainwindow.analytics_widget.create_figure()
            self.close()
            print("close")
